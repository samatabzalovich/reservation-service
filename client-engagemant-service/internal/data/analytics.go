...
func (a *AnalyticsModel) TotalAppointmentsOfInstitutionForGivenDateRange(institutionID int, startDate time.Time, endDate time.Time) (int, error) {
...
func (a *AnalyticsModel) WageOfEmployeeServiceForGivenDateRange(employeeID int, startDate time.Time, endDate time.Time) ([]*Wage, error) {
	stmt := `SELECT service_id,   COUNT(*) * s.price as wage FROM appointments a join services s on a.service_id = s.id WHERE employee_id = $1 AND a.created_at AND a.is_canceled = false BETWEEN $2 AND $3 group by a.service_id, s.price`
	var wage []*Wage
...
func (a *AnalyticsModel) TotalRevenueOfInstitutionForGivenDateRange(institutionID int, startDate time.Time, endDate time.Time) (Wage, error) {
	stmt := `SELECT service_id, COUNT(*) * s.price as wage FROM appointments a join services s on a.service_id = s.id WHERE a.institution_id = $1 AND a.created_at BETWEEN $2 AND $3 group by a.service_id, s.price`
	var wage Wage
